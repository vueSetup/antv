...
export default defineComponent({
  ...
  setup(props, { slots }) {
    ...
    const currentMenuActived = computed(
      () => context.menubarActived && state.active
    )
    ...
    const isPrevMenuHiddening = (e: MouseEvent) => {
      ...
      if (
        toElement &&
        toElement.className === `${context.prefixCls}-item-dropdown`
      ) {
        return true
      }
      ...
      for (let i = 0, l = childNodes.length; i < l; i += 1) {
        ...
        if (child.querySelector) {
          // TODO :: popupClassName
          const popupElem = child.querySelector(
            `${context.prefixCls}-item-dropdown`
          )!
